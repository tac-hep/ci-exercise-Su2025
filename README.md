...
main code has no dependencies other than Python 3.9+, while the tests require `pytest` and `uncertainties`.
...
* `pyproject.toml`: Setup a basic project. Include at least:
    ...
    * A `dependency-groups` table with a `test` and `dev` group (dependencies listed above). Note that this is different from the `project.optional-dependencies` table.
* `.pre-commit-config.yaml`: Add at least the following checks:
    ...
    * Ruff check with at least the default checks (E, F, W), bugbear (B), isort (I), Ruff-specific (RUF), and PyUpgrade (UP).
...
* `.github/workflows/ci.yml`: Add two jobs. For full credit, the code must pass both jobs. You will be able to do this without changing anything in `tests/` or `src/`.
    ...
    - Tests: run your tests. Either through nox or by hand, but again, use the `tests` group, don't list `pytest`, etc. Use `ubuntu-latest` and run both minimum and maximum supported Pythons (3.9 and 3.13), either with a matrix, or with nox, or both. If you implement it manually, remember pip needs to be 25.1+ to use `--group` (uv has supported it for a while).
